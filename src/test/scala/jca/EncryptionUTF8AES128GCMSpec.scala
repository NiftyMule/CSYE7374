package jca

import cats.effect.IO
import cats.effect.testing.scalatest.AsyncIOSpec
import cats.effect.unsafe.implicits.global
import org.scalatest.flatspec.AnyFlatSpec
import org.scalatest.freespec.AsyncFreeSpec
import org.scalatest.matchers.should
import tsec.cipher.symmetric.jca.{AES128GCM, SecretKey}

class EncryptionUTF8AES128GCMSpec extends AsyncFreeSpec with AsyncIOSpec with should.Matchers {

    "Encryption" - {

        implicit val encryptor: HexEncryption[AES128GCM] = EncryptionUTF8AES128GCM

        "buildKey" in {
            val keyIO: IO[Option[SecretKey[AES128GCM]]] = for {
                rawKey <- encryptor.genRawKey
                key <- encryptor.buildKey(rawKey)
            } yield Option(key)
            keyIO.asserting(ko => ko.isDefined shouldBe true)
        }

        "encrypt and decrypt" in {
            val originalMessage = "Hello, World!"
            val result: IO[String] = for {
                rawKey <- encryptor.genRawKey
                key <- encryptor.buildKey(rawKey)
                ciphertext <- encryptor.encrypt(key)(originalMessage)
                message <- encryptor.decrypt(key)(ciphertext)
            } yield message
            result.asserting(w => w shouldBe originalMessage)
        }

        "round-trip via Byte array" in {
            val originalMessage = "Hello, World!"
            val result: IO[String] =
                for {
                    rawKey <- encryptor.genRawKey
                    key <- encryptor.buildKey(rawKey)
                    ciphertext <- encryptor.encrypt(key)(originalMessage)
                    bytes <- encryptor.concat(ciphertext)
                    encrypted <- encryptor.bytesToCipherText(bytes)
                    message <- encryptor.decrypt(key)(encrypted)
                } yield message
            result.asserting(w => w shouldBe originalMessage)
        }

        "round-trip via Hex string" in {
            val originalMessage = "Hello, World!"
            val result: IO[String] =
                for {
                    rawKey <- encryptor.genRawKey
                    key <- encryptor.buildKey(rawKey)
                    ciphertext <- encryptor.encrypt(key)(originalMessage)
                    bytes1 <- encryptor.concat(ciphertext)
                    hex <- HexEncryption.bytesToHexString(bytes1)
                    bytes2 <- HexEncryption.hexStringToBytes(hex)
                    encrypted <- encryptor.bytesToCipherText(bytes2)
                    message <- encryptor.decrypt(key)(encrypted)
                } yield message
            result.asserting(w => w shouldBe originalMessage)
        }

        // TESTME why doesn't this work?
//        "decryptHex" in {
//            val encryption = implicitly[HexEncryption[AES128GCM]]
//            val result = encryption.decryptHex("k0JCcO$SY5OI50uj", "")
//            result.asserting(_ shouldBe "1,Leonhard Euler,Daniel Bernoulli,Isaac Newton,Srinivas Ramanujan,92.0,8.5,8.0,5.0,10.5,5.0,4.0,8.0,5.0,23.0,10.0,5.0,Presentation long and detailed.  Project excellent overall. Need to actually run UI myself.,https://github.com/youngbai/CSYE7200-MovieRecommendation")
//        }

        // TESTME why doesn't this work?
//        "decryptRow1" in {
//            val keyMap = Map("1" -> "k0JCcO$SY5OI50uj")
//            val row = Seq("1", "")
//            val wi: IO[String] = HexEncryption.decryptRow(keyMap)(row)
//            wi.asserting(_ shouldBe "1,Leonhard Euler,Daniel Bernoulli,Isaac Newton,Srinivas Ramanujan,92.0,8.5,8.0,5.0,10.5,5.0,4.0,8.0,5.0,23.0,10.0,5.0,Presentation long and detailed.  Project excellent overall. Need to actually run UI myself.,https://github.com/youngbai/CSYE7200-MovieRecommendation")
//        }

        "decryptRow2" in {
            val keyMap = Map("1" -> "k0JCcO$SY5OI50uj")
            val row = Seq("2", "")
            val wi: IO[String] = HexEncryption.decryptRow(keyMap)(row)
            wi.handleError { case _: EncryptionException => true }.asserting(_ shouldBe true)
        }

        "decryptRow2a" in {
            val keyMap = Map("1" -> "k0JCcO$SY5OI50uj")
            val row = Seq("2", "")
            val xi: IO[String] = HexEncryption.decryptRow(keyMap)(row)
            xi.handleError { case _: Exception => true; case _ => false }.asserting(_ shouldBe true)
        }

        "decryptRow3" in {
            val keyMap = Map("1" -> "QwSeQVJNuAg6D6H9")
            val row = Seq("1")
            val wi: IO[String] = HexEncryption.decryptRow(keyMap)(row)
            wi.handleError(_ => true).asserting(_ shouldBe true)
        }
    }
}
